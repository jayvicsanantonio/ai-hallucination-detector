...
export class ContradictionDetector {
  ...
  private extractStatements(content: ParsedContent): Statement[] {
    ...
    sentences.forEach((sentence) => {
      ...
      if (trimmedSentence.length > 10) {
        ...
        const sentenceEntities = content.entities.filter(
          (entity) =>
            entity.location.start >= startPos &&
  ...
  private findDirectContradictions(
    ...
    for (let i = 0; i < statements.length; i++) {
      for (let j = i + 1; j < statements.length; j++) {
        ...
        const antonymContradiction = this.checkAntonymContradiction(
          stmt1,
          stmt2
        );
      ...
      if (stmts.length > 1) {
        for (let i = 0; i < stmts.length; i++) {
          for (let j = i + 1; j < stmts.length; j++) {
            const contradiction = this.checkImplicitContradiction(
              stmts[i],
              stmts[j]
            );
            if (contradiction) {
  ...
  private findTemporalContradictions(
    ...
    const temporalStatements = statements.filter((stmt) =>
      this.hasTemporalIndicators(stmt.text)
    );

    for (let i = 0; i < temporalStatements.length; i++) {
      for (let j = i + 1; j < temporalStatements.length; j++) {
        const contradiction = this.checkTemporalContradiction(
  ...
  private findCausalContradictions(
    ...
    const causalStatements = statements.filter((stmt) =>
      this.hasCausalIndicators(stmt.text)
    );

    for (let i = 0; i < causalStatements.length; i++) {
      for (let j = i + 1; j < causalStatements.length; j++) {
        const contradiction = this.checkCausalContradiction(
          causalStatements[i],
          causalStatements[j]
        );
        if (contradiction) {
  ...
  private parseStatement(text: string): {
    subject?: string;
    predicate?: string;
    object?: string;
  } {
  ...
  private removeNegations(text: string): string {
    let cleanText = text;
    this.negationWords.forEach((negation) => {
      cleanText = cleanText.replace(
        new RegExp(`\\b${negation}\\b`, 'gi'),
        ''
      );
    });
  ...
  private calculateContextSimilarity(
    ...
    const entities1 = new Set(
      stmt1.entities.map((e) => e.value.toLowerCase())
    );
    const entities2 = new Set(
      stmt2.entities.map((e) => e.value.toLowerCase())
    );
    ...
    const textSimilarity =
      commonWords.size / Math.max(words1.size, words2.size);
  ...
  private hasCausalIndicators(text: string): boolean {
    const causalWords = [
      'because',
      'since',
      'due to',
  ...
  private extractTemporalInfo(text: string): any {
    // Simplified temporal extraction - would use NLP in production
    const temporalPattern =
      /(before|after|during|while|when|then|first|last|earlier|later)/gi;
    const matches = text.match(temporalPattern);
    return matches ? { indicators: matches } : null;
  }

  private extractCausalInfo(text: string): any {
    // Simplified causal extraction - would use NLP in production
    const causalPattern =
      /(because|since|due to|caused by|results in|leads to|therefore|thus)/gi;
