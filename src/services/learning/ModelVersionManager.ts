import { ModelVersion, ABTestConfig } from './MLModelTrainer';
...
export class ModelVersionManager {
  ...
  private activeVersions: Map<string, string> = new Map(); // domain -> versionId

  constructor() {
    this.logger = new Logger('ModelVersionManager');
  }
  ...
  async deployModel(
    ...
    try {
      ...
      const deployment: DeploymentStatus = {
        ...
        status: 'pending',
        rolloutPercentage:
          config.rolloutStrategy === 'immediate'
            ? 100
      ...
      setTimeout(() => {
        this.executeDeployment(deploymentId, config).catch(
          (error) => {
  ...
  async listDeployments(options?: {
    ...
    if (options?.environment) {
      deployments = deployments.filter(
        (d) => d.environment === options.environment
      );
    }

    if (options?.status) {
      deployments = deployments.filter(
        (d) => d.status === options.status
      );
    }
   ...
   */
  async rollbackModel(plan: RollbackPlan): Promise<DeploymentStatus> {
    try {
   ...
   */
  async getActiveVersion(domain: Domain): Promise<string | null> {
   ...
   */
  private async validateModelVersion(
    versionId: string
  ): Promise<void> {
  ...
  private async executeGradualRollout(
    ...
    for (const percentage of rolloutSteps) {
      if (percentage <= (config.rolloutPercentage || 10)) continue;
      ...
      if (healthCheck.status === 'unhealthy') {
        throw new Error(
          `Health check failed at ${percentage}% rollout`
        );
      }
  ...
  private shouldTriggerRollback(
    ...
    healthCheck: { status: string; metrics: Record<string, number> },
    threshold: { accuracyDrop: number; errorRateIncrease: number }
  ): boolean {
    if (healthCheck.status === 'unhealthy') {
      return true;
    }
