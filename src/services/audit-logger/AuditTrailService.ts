...
} from '../../database/interfaces/AuditRepository';
import {
  AuditEntry,
  AuditQuery,
  AuditSummary,
...
export class AuditTrailService {
  ...
  private retentionRepository: AuditRetentionRepository;
  ...
  async setRetentionPolicy(
    ...
    try {
      const existingPolicy =
        await this.retentionRepository.getRetentionPolicy(
          organizationId
        );

      if (existingPolicy) {
        return await this.retentionRepository.updateRetentionPolicy(
          organizationId,
          {
            retentionYears,
            autoArchive,
  ...
  async getRetentionPolicy(
    ...
    return this.retentionRepository.getRetentionPolicy(
      organizationId
    );
  }
   ...
   */
  async getEffectiveRetentionYears(
    organizationId: string
  ): Promise<number> {
    const policy = await this.getRetentionPolicy(organizationId);
    return (
      policy?.retentionYears ?? this.config.defaultRetentionYears
    );
  }
   ...
   */
  async persistAuditEntry(entry: AuditEntry): Promise<AuditEntry> {
    try {
   ...
   */
  async persistAuditEntriesBatch(
    entries: AuditEntry[]
  ): Promise<AuditEntry[]> {
    if (entries.length === 0) {
      return [];
    ...
    try {
      ...
      for (
        ...
        const batchResults =
          await this.auditRepository.createAuditEntriesBatch(batch);
        results.push(...batchResults);
      }

      this.logger.debug(
        `Persisted ${entries.length} audit entries in ${Math.ceil(
          entries.length / this.config.maxBatchSize
        )} batches`
      );
      return results;
    } catch (error) {
  ...
  async getAuditSummary(
    ...
    try {
      return await this.auditRepository.getAuditSummary(
        organizationId,
        startDate,
        endDate
      );
  ...
  async getAuditTrailMetrics(
    ...
    try {
      ...
      const [totalEntries, entriesLastHour, entriesLastDay, summary] =
        await Promise.all([
      ...
      return {
        ...
        storageUsageMB: this.estimateStorageUsage(
   ...
   */
  async archiveOldData(organizationId: string): Promise<number> {
    try {
      const retentionYears = await this.getEffectiveRetentionYears(
        organizationId
      );
      const archiveBeforeDate = new Date();
      archiveBeforeDate.setFullYear(
        archiveBeforeDate.getFullYear() - retentionYears
      );

      const archivedCount =
        await this.auditRepository.archiveOldSessions(
          organizationId,
          archiveBeforeDate
        );
   ...
   */
  async cleanupExpiredData(organizationId: string): Promise<number> {
    try {
      const retentionYears = await this.getEffectiveRetentionYears(
        organizationId
      );
  ...
  async runScheduledCleanup(): Promise<void> {
    try {
      const policies =
        await this.retentionRepository.getAllRetentionPolicies();
      const autoArchivePolicies = policies.filter(
        (p) => p.autoArchive
      );

      this.logger.info(
        `Running scheduled cleanup for ${autoArchivePolicies.length} organizations`
      );
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!entry.success && !entry.errorMessage) {
          issues.push(
            `Entry ${entry.id} marked as failed but no error message`
          );
        }
      }
  ...
  private validateAuditEntry(entry: AuditEntry): void {
    if (!entry.id) {
  ...
  private estimateStorageUsage(entryCount: number): number {
