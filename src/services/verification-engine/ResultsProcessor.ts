...
export class ResultsProcessor {
  ...
  constructor(config: ProcessorConfig = {}) {
    this.logger = new Logger('ResultsProcessor');
    this.config = {
  ...
  async processResults(
    ...
    try {
      ...
      if (this.config.enableCaching) {
        ...
        if (cachedResult) {
          this.logger.debug(
            `Returning cached result for verification ${verificationId}`
          );
          return {
            ...cachedResult,
      ...
      const aggregatedResult = await this.aggregateModuleResults(
        verificationId,
        request,
        moduleResults,
        processingTime
      ...
      const weightedResult = this.applyConfidenceWeighting(
        aggregatedResult,
        request.domain
      );
      ...
      this.logger.debug(
        `Results processed in ${totalProcessingTime}ms for verification ${verificationId}`
      );

      return formattedResult;
    } catch (error) {
      this.logger.error(
        `Error processing results for verification ${verificationId}:`,
        error
      );
      throw error;
    }
  ...
  private async aggregateModuleResults(
    verificationId: string,
    request: VerificationRequest,
    moduleResults: ValidationResult[],
    processingTime: number
    ...
    moduleResults.forEach((result) => {
      ...
      if (result.issues.length > 0) {
        recommendations.push(
          `${result.moduleId} module detected ${result.issues.length} issue(s)`
        );
      }
    });
  ...
  private applyConfidenceWeighting(
    ...
    const weightedConfidence = Math.min(
      100,
      Math.max(0, result.overallConfidence * weight)
    );
  ...
  private generateRecommendations(
    ...
    if (issuesByType.factual_error) {
      recommendations.push(
        `${issuesByType.factual_error.length} factual error(s) detected. Review and verify against authoritative sources.`
      );
    }

    if (issuesByType.logical_inconsistency) {
      recommendations.push(
        `${issuesByType.logical_inconsistency.length} logical inconsistency(ies) found. Check for contradictions and ensure coherent reasoning.`
      );
    }

    if (issuesByType.compliance_violation) {
      recommendations.push(
        `${issuesByType.compliance_violation.length} compliance violation(s) identified. Consult with legal/compliance team before proceeding.`
      );
    }
    ...
    switch (riskLevel) {
      case 'critical':
        recommendations.push(
          'CRITICAL: Do not use this content without thorough review and correction.'
        );
        break;
      case 'high':
        recommendations.push(
          'HIGH RISK: Significant issues detected. Manual review strongly recommended.'
        );
        break;
      case 'medium':
        recommendations.push(
          'MEDIUM RISK: Some issues detected. Consider review before use.'
        );
        break;
    }
  ...
  private generateCacheKey(request: VerificationRequest): string {
    ...
    return this.cache.generateCacheKey(
      contentHash,
      request.domain,
      options
    );
  ...
  private async persistResult(
    ...
    this.logger.debug(
      `Persisting result for verification ${result.verificationId}`
    );
  ...
  private async getPersistedResult(
    ...
    this.logger.debug(
      `Retrieving persisted result for verification ${verificationId}`
    );
    return null;
  }
