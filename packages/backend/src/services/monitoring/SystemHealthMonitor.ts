...
export class SystemHealthMonitor {
  ...
  getCurrentMetrics(): {
    ...
    return {
      ...
      application:
        this.appMetricsHistory[this.appMetricsHistory.length - 1] ||
        null,
    };
  }
  ...
  getMetricsHistory(
    ...
    return {
      system: this.metricsHistory.filter(
  ...
  private async checkMemoryHealth(): Promise<HealthCheck> {
    ...
    if (memoryUsage > this.config.alertThresholds.memory.critical) {
      ...
      memoryUsage > this.config.alertThresholds.memory.warning
    ) {
      return {
        name: 'memory',
        status: 'warn',
  ...
  private async checkApiHealth(): Promise<HealthCheck> {
    ...
    if (
      errorRate > this.config.alertThresholds.errorRate.critical ||
      avgResponseTime >
      ...
      return {
        ...
        message: `API performance critical: ${errorRate.toFixed(
          1
        )}% error rate, ${avgResponseTime}ms avg response`,
      };
    } else if (
      errorRate > this.config.alertThresholds.errorRate.warning ||
      avgResponseTime >
      ...
      return {
        ...
        status: 'warn',
        responseTime: avgResponseTime,
        message: `API performance degraded: ${errorRate.toFixed(
          1
        )}% error rate, ${avgResponseTime}ms avg response`,
      };
    }
    ...
    return {
      ...
      status: 'pass',
      responseTime: avgResponseTime,
      message: `API performance normal: ${errorRate.toFixed(
        1
      )}% error rate, ${avgResponseTime}ms avg response`,
    };
  }
  ...
  private cleanupOldMetrics(): void {
    ...
    this.appMetricsHistory = this.appMetricsHistory.filter(
      (m) => m.timestamp > cutoffTime
    );
  }
