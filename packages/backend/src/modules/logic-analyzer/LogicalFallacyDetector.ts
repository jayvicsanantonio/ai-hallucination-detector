...
export class LogicalFallacyDetector {
  private fallacyPatterns: FallacyPattern[] = [
    {
      name: 'Ad Hominem',
      description:
      ...
      severity: 'medium',
      explanation:
        'Focus on addressing the argument rather than personal characteristics',
      examples: [
    ...
    },
    {
      ...
      severity: 'medium',
      explanation:
        'Address the actual argument being made, not a distorted version',
      examples: [
    ...
    },
    {
      ...
      severity: 'medium',
      explanation:
        'Consider additional alternatives beyond the two presented',
      examples: [
    ...
    },
    {
      ...
      severity: 'low',
      explanation:
        'Evaluate the evidence and reasoning, not just the source',
      examples: ['It must be true because the CEO said so'],
    },
    {
      name: 'Slippery Slope',
      description:
        'Arguing that one event will lead to a chain of negative consequences',
      patterns: [
      ...
      severity: 'medium',
      explanation:
        'Provide evidence for each step in the proposed chain of events',
      examples: [
    ...
    },
    {
      ...
      severity: 'high',
      explanation:
        'Provide independent evidence rather than restating the conclusion',
    ...
    },
    {
      ...
      severity: 'medium',
      explanation:
        'Ensure sufficient sample size before making generalizations',
      examples: [
    ...
    },
    {
      ...
      severity: 'medium',
      explanation:
        'Focus on logical arguments rather than emotional appeals',
      examples: [
    ...
    },
    {
      ...
      severity: 'low',
      explanation:
        'Evaluate the evidence rather than relying on popularity',
    ...
    try {
      ...
      for (const sentence of sentences) {
        for (const pattern of this.fallacyPatterns) {
          const detectedFallacy = this.checkForFallacy(
            sentence,
            pattern,
            text
          );
  ...
  private createFallacy(
    ...
    pattern: FallacyPattern,
    fullText: string,
    detectionMethod: 'pattern' | 'keyword'
  ): LogicalFallacy {
  ...
  private deduplicateFallacies(
    ...
    const deduplicated: LogicalFallacy[] = [];

    for (const fallacy of fallacies) {
      const isDuplicate = deduplicated.some(
        (existing) =>
          existing.type === fallacy.type &&
      ...
      if (!isDuplicate) {
        ...
        const existingIndex = deduplicated.findIndex(
          (existing) =>
            existing.type === fallacy.type &&
            Math.abs(
              existing.location.start - fallacy.location.start
            ) < 50
        );
  ...
  getFallacyPattern(fallacyType: string): FallacyPattern | undefined {
    return this.fallacyPatterns.find(
      (pattern) => pattern.name === fallacyType
    );
  }
}
