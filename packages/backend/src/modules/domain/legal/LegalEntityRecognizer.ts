...
export class LegalEntityRecognizer {
  private entityPatterns: Map<string, RegExp> = new Map();
  private jurisdictionPatterns: Map<string, RegExp> = new Map();
  ...
  private extractCorporations(text: string): LegalEntity[] {
    const entities: LegalEntity[] = [];
    const corporationPattern =
      /([A-Z][a-zA-Z\s&.]*?)\s+(Corporation|Company|Inc\.?|Corp\.?)\b/g;
  ...
  private extractLLCs(text: string): LegalEntity[] {
    const entities: LegalEntity[] = [];
    // More precise pattern that looks for entity names before LLC
    const llcPattern =
      /\b([A-Z][a-zA-Z]*(?:\s+[A-Z&][a-zA-Z]*){0,4})\s+(LLC|L\.L\.C\.?|Limited Liability Company)\b/g;
  ...
  private extractPartnerships(text: string): LegalEntity[] {
    const entities: LegalEntity[] = [];
    const partnershipPattern =
      /([A-Z][a-zA-Z\s&]+?)(?:\s+(?:LP|L\.P\.?|LLP|L\.L\.P\.?|Partnership|Partners))/g;
  ...
  private extractIndividuals(text: string): LegalEntity[] {
    const entities: LegalEntity[] = [];

    // Pattern for individual names (First Last or First Middle Last)
    const individualPattern =
      /(?:Mr\.?|Mrs\.?|Ms\.?|Dr\.?)?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]*\.?)?\s+[A-Z][a-z]+)(?!\s+(?:Inc\.?|Corp\.?|LLC|LP|LLP|Company|Corporation))/g;
  ...
  private extractGovernmentEntities(text: string): LegalEntity[] {
    const entities: LegalEntity[] = [];
    const governmentPattern =
      /(?:United States|U\.S\.|State of [A-Z][a-z]+|City of [A-Z][a-z]+|County of [A-Z][a-z]+|[A-Z][a-z]+\s+(?:Department|Agency|Bureau|Commission|Authority))/g;
    ...
    while ((match = governmentPattern.exec(text)) !== null) {
      entities.push({
        name: match[0].trim(),
        type: 'government',
        jurisdiction: this.extractJurisdictionFromGovernmentEntity(
          match[0]
        ),
        location: {
          start: match.index,
  ...
  private enhanceWithJurisdiction(
    ...
    for (const entity of entities) {
      ...
      entity.registrationNumber = this.extractRegistrationNumber(
        entity,
        text
      );
    }
  ...
  private detectJurisdiction(
    ...
    const entityName = entity.name.replace(
      /[.*+?^${}()|[\]\\]/g,
      '\\$&'
    ); // Escape regex chars

    // Look for patterns like "EntityName incorporated in State" or "EntityName organized under State law"
    const entityJurisdictionPattern = new RegExp(
      `${entityName}\\s+(?:incorporated in|organized under)\\s+([a-z\\s]+)(?:\\s+law)?`,
      'i'
    );
    ...
    const afterEntityText = text.substring(
      entity.location.end,
      Math.min(text.length, entity.location.end + 50)
    );

    // Check for "organized under [state] law" pattern after the entity
    const organizedUnderMatch = afterEntityText.match(
      /\s+organized under\s+([a-z\s]+)\s+law/i
    );
    ...
    const incorporatedInMatch = afterEntityText.match(
      /\s+incorporated in\s+([a-z\s]+)/i
    );
    ...
    const immediateContext = text.substring(
      Math.max(0, entity.location.start - 30),
      Math.min(text.length, entity.location.end + 30)
    );
  ...
  private extractRegistrationNumber(
    ...
    const entityText = text.substring(
      Math.max(0, entity.location.start - 100),
      Math.min(text.length, entity.location.end + 100)
    );
  ...
  private extractJurisdictionFromGovernmentEntity(
    ...
    if (
      entityName.includes('United States') ||
  ...
  private initializeEntityPatterns(): void {
    this.entityPatterns.set(
      'corporation',
      /\b[A-Z][a-zA-Z\s&]+(?:Inc\.?|Corp\.?|Corporation|Company)\b/g
    );
    this.entityPatterns.set(
      'llc',
      /\b[A-Z][a-zA-Z\s&]+(?:LLC|L\.L\.C\.?|Limited Liability Company)\b/g
    );
    this.entityPatterns.set(
      'partnership',
      /\b[A-Z][a-zA-Z\s&]+(?:LP|L\.P\.?|LLP|L\.L\.P\.?|Partnership)\b/g
    );
    this.entityPatterns.set(
      'individual',
      /(?:Mr\.?|Mrs\.?|Ms\.?|Dr\.?)?\s*[A-Z][a-z]+(?:\s+[A-Z][a-z]*\.?)?\s+[A-Z][a-z]+/g
    );
  }

  private initializeJurisdictionPatterns(): void {
    this.jurisdictionPatterns.set(
      'delaware',
      /(?:Delaware|DE|incorporated in Delaware|Delaware corporation)/i
    );
    this.jurisdictionPatterns.set(
      'california',
      /(?:California|CA|incorporated in California|California corporation)/i
    );
    this.jurisdictionPatterns.set(
      'new york',
      /(?:New York|NY|incorporated in New York|New York corporation)/i
    );
    this.jurisdictionPatterns.set(
      'texas',
      /(?:Texas|TX|incorporated in Texas|Texas corporation)/i
    );
    this.jurisdictionPatterns.set(
      'florida',
      /(?:Florida|FL|incorporated in Florida|Florida corporation)/i
    );
    this.jurisdictionPatterns.set(
      'federal',
      /(?:United States|U\.S\.|federal|Federal)/i
    );
  }
  ...
  private deduplicateEntities(
    ...
    const deduplicated: LegalEntity[] = [];
