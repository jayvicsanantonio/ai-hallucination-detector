...
class MetadataExtractor {
     ...
     */
    extractMetadata(parsedContent, options = {}) {
        const { extractTitle = true, extractAuthor = true, extractKeywords = true, extractTopics = true, calculateReadability = true, analyzeSentiment = false, analyzeTone = false, extractEntities = true, maxKeywords = 10, maxTopics = 5, } = options;
        const metadata = {};
        ...
        if (calculateReadability) {
            ...
            metadata.complexity = this.determineComplexity(metadata.readabilityScore);
        }
        // Analyze sentiment
        if (analyzeSentiment) {
        ...
        if (extractEntities) {
            metadata.entitySummary = this.summarizeEntities(parsedContent.entities);
        }
     ...
     */
    extractTitle(text, parsedContent) {
     ...
     */
    extractAuthor(text, parsedContent) {
        ...
        const personEntities = parsedContent.entities.filter((e) => e.type === 'person');
    ...
    extractTopics(text, maxTopics) {
        ...
        for (const keyword of keywords) {
            ...
            const relatedKeywords = keywords.filter((k) => !usedKeywords.has(k) && this.areRelated(keyword, k, text));
     ...
     */
    determineComplexity(readabilityScore) {
    ...
    analyzeTone(text) {
        ...
        const formalScore = formalIndicators.reduce((score, word) => score + (words.includes(word) ? 1 : 0), 0);
        const informalScore = informalIndicators.reduce((score, word) => score + (words.includes(word) ? 1 : 0), 0);
        const technicalScore = technicalIndicators.reduce((score, word) => score + (words.includes(word) ? 1 : 0), 0);
        const conversationalScore = conversationalIndicators.reduce((score, word) => score + (words.includes(word) ? 1 : 0), 0);
        const maxScore = Math.max(formalScore, informalScore, technicalScore, conversationalScore);
        if (maxScore === technicalScore)
            return 'technical';
        if (maxScore === formalScore)
            return 'formal';
        if (maxScore === conversationalScore)
     ...
     */
    calculateCompleteness(parsedContent) {
        let score = 0;
        // Check for basic structure
        if (parsedContent.extractedText.length > 100)
            score += 0.2;
        if (parsedContent.structure.sections.length > 0)
            score += 0.2;
        if (parsedContent.entities.length > 0)
            score += 0.2;
        // Check for rich content
        if (parsedContent.structure.tables.length > 0)
            score += 0.1;
        if (parsedContent.structure.figures.length > 0)
            score += 0.1;
        if (parsedContent.structure.references.length > 0)
            score += 0.2;
    ...
    calculateCoherence(text) {
        ...
        const transitionScore = Math.min(1, transitionCount / (sentences.length * 0.1));
     ...
     */
    extractCategories(text, parsedContent) {
        const categories = [];
        const lowerText = text.toLowerCase();
        // Business/Finance
        if (/\b(?:business|finance|money|profit|revenue|investment|market|economy)\b/.test(lowerText)) {
            categories.push('business');
        }
        // Technology
        if (/\b(?:technology|software|computer|digital|internet|data|algorithm|programming)\b/.test(lowerText)) {
            categories.push('technology');
        }
        // Healthcare
        if (/\b(?:health|medical|doctor|patient|treatment|medicine|hospital|disease)\b/.test(lowerText)) {
            categories.push('healthcare');
        }
        // Legal
        if (/\b(?:legal|law|court|contract|agreement|regulation|compliance|attorney)\b/.test(lowerText)) {
            categories.push('legal');
        }
        // Education
        if (/\b(?:education|school|university|student|teacher|learning|academic|research)\b/.test(lowerText)) {
     ...
     */
    extractTags(text, parsedContent) {
        const tags = new Set();
