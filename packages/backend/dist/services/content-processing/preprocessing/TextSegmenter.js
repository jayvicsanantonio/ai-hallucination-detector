...
exports.TextSegmenter = void 0;
class TextSegmenter {
    /**
     * Segment text based on provided options
     */
    segment(content, options = {}) {
        const { segmentBySentences = false, segmentByParagraphs = true, segmentBySemanticBlocks = false, maxSegmentLength = 1000, minSegmentLength = 50, overlapLength = 0, preserveStructure = true, } = options;
        const segments = [];
        let segmentIndex = 0;
        ...
        if (segments.length === 0 ||
            ...
            const chunks = this.createChunks(content, maxSegmentLength, minSegmentLength, overlapLength);
        ...
        const statistics = {
            ...
            paragraphs: segments.filter((s) => s.type === 'paragraph')
                .length,
            semanticBlocks: segments.filter((s) => s.type === 'semantic_block').length,
            chunks: segments.filter((s) => s.type === 'chunk').length,
        };
        const totalLength = segments.reduce((sum, s) => sum + s.text.length, 0);
        const averageLength = segments.length > 0 ? totalLength / segments.length : 0;
        return {
            segments,
     ...
     */
    createChunks(content, maxLength, minLength, overlapLength) {
        const chunks = [];
     ...
     */
    segmentParsedContent(parsedContent, options = {}) {
        const segmentationResult = this.segment(parsedContent.extractedText, options);
        return {
            ...parsedContent,
     ...
     */
    getOptimalSegmentation(contentLength) {
     ...
     */
    mergeSmallSegments(segments, minLength) {
        const merged = [];
...
exports.TextSegmenter = TextSegmenter;
