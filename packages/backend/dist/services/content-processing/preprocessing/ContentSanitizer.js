...
class ContentSanitizer {
    ...
    sanitize(content, options = {}) {
        const { removeHtml = false, removeMarkdown = false, normalizeWhitespace = true, removeEmptyLines = true, trimLines = true, convertToLowercase = false, removeSpecialCharacters = false, preserveStructure = true, maxLineLength, } = options;
        ...
        if (maxLineLength && maxLineLength > 0) {
            sanitizedText = sanitizedText
                ...
                .join('\n');
            transformations.push(`Lines limited to ${maxLineLength} characters`);
        }
     ...
     */
    sanitizeParsedContent(parsedContent, options = {}) {
        const result = this.sanitize(parsedContent.extractedText, options);
        return {
            ...parsedContent,
    ...
    removeSensitiveInfo(content) {
        ...
        if (emails) {
            ...
            sanitizedText = sanitizedText.replace(emailRegex, '[EMAIL_REDACTED]');
        ...
        if (phones) {
            ...
            sanitizedText = sanitizedText.replace(phoneRegex, '[PHONE_REDACTED]');
        ...
        if (ssns) {
            ...
            sanitizedText = sanitizedText.replace(ssnRegex, '[SSN_REDACTED]');
        ...
        if (creditCards) {
            ...
            sanitizedText = sanitizedText.replace(ccRegex, '[CREDIT_CARD_REDACTED]');
