...
const SOC2ComplianceService_1 = require("./SOC2ComplianceService");
const GDPRComplianceService_1 = require("./GDPRComplianceService");
class ComplianceFrameworkService {
    constructor(soc2Service, gdprService, config) {
        ...
        const supportedFrameworks = [
            {
                ...
                enabled: this.configuration.enabledFrameworks.includes('SOC2'),
                status: 'not-assessed',
            },
            {
                id: 'GDPR',
                ...
                enabled: this.configuration.enabledFrameworks.includes('GDPR'),
                status: 'not-assessed',
            },
            {
                id: 'HIPAA',
                ...
                enabled: this.configuration.enabledFrameworks.includes('HIPAA'),
                status: 'not-assessed',
            },
            {
                id: 'PCI-DSS',
                ...
                enabled: this.configuration.enabledFrameworks.includes('PCI-DSS'),
                status: 'not-assessed',
            },
            {
                id: 'ISO27001',
                ...
                enabled: this.configuration.enabledFrameworks.includes('ISO27001'),
                status: 'not-assessed',
            },
        ];
    ...
    async performComplianceAssessment() {
        ...
        if (this.isFrameworkEnabled('SOC2')) {
            try {
                ...
                soc2Assessment.findings.forEach((finding) => {
                    ...
                    this.complianceIssues.set(`SOC2-${finding.controlId}`, issue);
                });
                ...
                recommendations.push('SOC 2 assessment failed - manual review required');
            }
        }
            ...
            try {
                ...
                if (gdprReport.expiredData > 0) {
                    const issue = {
                        ...
                        remediation: 'Perform data retention cleanup to remove or anonymize expired data',
                        status: 'open',
                    };
                    criticalIssues.push(issue);
                ...
                if (gdprReport.pendingRequests > 5) {
                    const issue = {
                        ...
                        remediation: 'Process pending requests within 30-day legal requirement',
                ...
                recommendations.push('GDPR assessment failed - manual review required');
            }
        }
        // Calculate overall compliance score
        const overallComplianceScore = assessedFrameworks > 0
        ...
        if (overallComplianceScore < 70) {
            ...
            recommendations.push('Consider engaging compliance consultant for comprehensive review');
        }
        else if (overallComplianceScore < 90) {
            recommendations.push('Good compliance posture but room for improvement');
            recommendations.push('Focus on addressing medium and high severity issues');
        }
        const report = {
            reportId,
            generatedDate,
    ...
    async performAutomatedRemediation() {
        ...
        if (this.isFrameworkEnabled('GDPR')) {
            try {
                results.attempted++;
                const cleanupResults = await this.gdprService.performRetentionCleanup();
                if (cleanupResults.errors.length === 0) {
                    results.successful++;
                    // Update compliance issue status
                    const issue = this.complianceIssues.get('GDPR-expired-data');
                    if (issue) {
                ...
                results.failed++;
                results.errors.push(`GDPR remediation failed: ${error.message}`);
            }
        }
        return results;
    }
    ...
    getComplianceStatus() {
        ...
        const criticalIssues = Array.from(this.complianceIssues.values()).filter((issue) => issue.severity === 'critical' && issue.status === 'open').length;
        return {
            frameworks: Array.from(this.frameworks.values()),
     ...
     */
    resolveComplianceIssue(issueId, resolution) {
    ...
    isFrameworkEnabled(frameworkId) {
    ...
    async sendComplianceNotifications(report) {
    ...
    async sendEmailNotification(email, report) {
        // Email notification implementation would go here
        console.log(`Sending compliance report ${report.reportId} to ${email}`);
    }
}
exports.ComplianceFrameworkService = ComplianceFrameworkService;
exports.complianceFrameworkService = new ComplianceFrameworkService(new SOC2ComplianceService_1.SOC2ComplianceService(), new GDPRComplianceService_1.GDPRComplianceService());
//# sourceMappingURL=ComplianceFrameworkService.js.map