...
const KnowledgeBaseRepository_1 = require("../../database/interfaces/KnowledgeBaseRepository");
const Logger_1 = require("../../utils/Logger");
class KnowledgeBaseUpdater {
    constructor(knowledgeRepository) {
        this.knowledgeRepository =
            knowledgeRepository || new KnowledgeBaseRepository_1.KnowledgeBaseRepository();
        this.logger = new Logger_1.Logger('KnowledgeBaseUpdater');
    }
    /**
    ...
    async updateFromFeedback(feedbackData) {
        try {
            ...
            const verificationResult = await this.knowledgeRepository.getVerificationResult(feedbackData.verificationId);
            ...
            if (feedbackData.userFeedback === 'incorrect') {
                updates.push(...(await this.processIncorrectFeedback(feedbackData, verificationResult)));
            }
            else if (feedbackData.userFeedback === 'correct') {
                updates.push(...(await this.processCorrectFeedback(feedbackData, verificationResult)));
            }
            else if (feedbackData.userFeedback === 'partially_correct') {
                updates.push(...(await this.processPartialFeedback(feedbackData, verificationResult)));
            }
            ...
            this.logger.error('Error updating knowledge base from feedback', {
     ...
     */
    async processIncorrectFeedback(feedbackData, verificationResult) {
        const updates = [];
        try {
            ...
            if (verificationResult.issues &&
                ...
                for (const issue of verificationResult.issues) {
                    ...
                    if (issue.type === 'factual_error' &&
                        ...
                        await this.knowledgeRepository.createOrUpdateFactualClaim(factualClaim);
                        updates.push({
                            type: 'factual_claim',
                            action: 'create',
     ...
     */
    async processCorrectFeedback(feedbackData, verificationResult) {
        const updates = [];
        try {
     ...
     */
    async processPartialFeedback(feedbackData, verificationResult) {
        const updates = [];
        try {
            // Handle partial corrections
            if (feedbackData.corrections && feedbackData.expertNotes) {
                // Create nuanced rules based on expert notes
                await this.createNuancedRule(feedbackData, verificationResult);
                updates.push({
                    type: 'pattern_rule',
                    action: 'create',
                    confidence: 0.6,
     ...
     */
    async updateComplianceRule(issue, feedbackData) {
        try {
            // Find the compliance rule that triggered this issue
            const rule = await this.knowledgeRepository.findComplianceRuleByIssue(issue);
            if (rule) {
                ...
                await this.knowledgeRepository.updateComplianceRule(rule.id, updatedRule);
            }
        }
        catch (error) {
     ...
     * Reinforce existing knowledge that was correctly identified
     */
    async reinforceKnowledge(issue, feedbackData) {
        try {
            if (issue.type === 'factual_error') {
                // Increase confidence in factual claims that were correctly identified
                await this.knowledgeRepository.reinforceFactualClaim(issue.evidence);
            }
            else if (issue.type === 'compliance_violation') {
                // Increase confidence in compliance rules
                await this.knowledgeRepository.reinforceComplianceRule(issue.ruleId);
            }
        }
        catch (error) {
     ...
     */
    async createNuancedRule(feedbackData, verificationResult) {
        try {
...
//# sourceMappingURL=KnowledgeBaseUpdater.js.map