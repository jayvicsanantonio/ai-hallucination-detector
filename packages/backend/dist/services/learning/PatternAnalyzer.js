...
class PatternAnalyzer {
    constructor(feedbackRepository) {
        ...
        this.logger = new Logger_1.Logger('PatternAnalyzer');
    }
    /**
     * Analyze new feedback for patterns
     */
    async analyzeNewFeedback(feedbackData) {
        try {
            const patterns = [];
            // Analyze correction patterns
            if (feedbackData.corrections) {
                patterns.push(...this.extractCorrectionPatterns(feedbackData.corrections));
            }
            // Analyze expert notes patterns
            if (feedbackData.expertNotes) {
                patterns.push(...this.extractExpertNotePatterns(feedbackData.expertNotes));
            }
            // Analyze temporal patterns
            patterns.push(...(await this.analyzeTemporalPatterns(feedbackData)));
    ...
    async analyzeFeedbackPatterns(options) {
        try {
            ...
            const feedbackData = await this.feedbackRepository.getFeedbackForAnalysis({
                domain: domain,
                timeframe,
                userId,
            });
            // Analyze trends
            const trends = await this.analyzeTrends(feedbackData, timeframe);
            // Find common corrections
            const commonCorrections = this.findCommonCorrections(feedbackData);
     ...
     */
    extractExpertNotePatterns(expertNotes) {
        const patterns = [];
     ...
     */
    async analyzeTemporalPatterns(feedbackData) {
        const patterns = [];
        try {
            // Get recent feedback for the same verification type
            const recentFeedback = await this.feedbackRepository.getRecentFeedback(feedbackData.verificationId, '7d');
            // Check for recurring issues
            const feedbackTypes = recentFeedback.map((f) => f.userFeedback);
            const incorrectCount = feedbackTypes.filter((t) => t === 'incorrect').length;
            if (incorrectCount > recentFeedback.length * 0.5) {
                patterns.push('recurring_accuracy_issue');
            }
            // Check for improvement trends
            const chronologicalFeedback = recentFeedback.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
            if (chronologicalFeedback.length >= 3) {
                const recentAccuracy = this.calculateAccuracy(chronologicalFeedback.slice(-3));
                const olderAccuracy = this.calculateAccuracy(chronologicalFeedback.slice(0, 3));
     ...
     */
    async analyzeTrends(feedbackData, timeframe) {
        const trends = [];
        try {
            ...
            for (const [period, data] of Object.entries(periods)) {
                ...
                const previousPeriod = this.getPreviousPeriod(period, timeframe);
     ...
     */
    findCommonCorrections(feedbackData) {
     ...
     */
    identifyImprovementAreas(feedbackData) {
        const areas = [];
        // Analyze feedback types
        const incorrectCount = feedbackData.filter((f) => f.userFeedback === 'incorrect').length;
        const partialCount = feedbackData.filter((f) => f.userFeedback === 'partially_correct').length;
        const total = feedbackData.length;
        if (total === 0)
            return areas;
        if (incorrectCount / total > 0.3) {
            areas.push('High false positive rate - review detection algorithms');
        }
        if (partialCount / total > 0.2) {
            areas.push('Many partial matches - improve context understanding');
        }
        ...
        if (expertNotes.filter((note) => note.includes('context')).length >
            expertNotes.length * 0.3) {
            areas.push('Context sensitivity needs improvement');
        }
        if (expertNotes.filter((note) => note.includes('outdated')).length >
    ...
    calculateAccuracy(feedbackData) {
        if (feedbackData.length === 0)
            return 0;
        const correctCount = feedbackData.filter((f) => f.userFeedback === 'correct').length;
        const partialCount = feedbackData.filter((f) => f.userFeedback === 'partially_correct').length;
     ...
     */
    groupByTimePeriod(feedbackData, timeframe) {
        const groups = {};
        feedbackData.forEach((feedback) => {
    ...
    getPeriodKey(date, timeframe) {
        if (timeframe.includes('d')) {
            ...
            return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; // Monthly
        }
    }
    /**
     * Get previous period key
     */
    getPreviousPeriod(period, timeframe) {
