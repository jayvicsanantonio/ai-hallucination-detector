...
class MLModelTrainer {
    constructor() {
        ...
        this.logger = new Logger_1.Logger('MLModelTrainer');
    }
    /**
     ...
     */
    async getActiveModelVersion(domain) {
        const versions = Array.from(this.modelVersions.values());
        return (versions.find((v) => v.domain === domain && v.isActive) || null);
    }
    /**
     * Deploy a model version (make it active)
     */
    async deployModelVersion(versionId) {
        try {
     ...
     */
    async updateABTestMetrics(testId, modelVersion, metrics) {
        try {
            const abTest = this.abTests.get(testId);
            if (!abTest) {
                throw new Error(`A/B test ${testId} not found`);
            }
            const targetMetrics = modelVersion === 'A'
     ...
     */
    async executeTraining(jobId, options) {
        const job = this.trainingJobs.get(jobId);
        if (!job)
            return;
        try {
            ...
            const { trainingSet, validationSet } = this.prepareTrainingData(options.trainingData, options.validationSplit || 0.2);
            job.progress = 30;
            // Initialize model
            this.logger.info('Initializing model', { jobId });
            const model = await this.initializeModel(options.domain, options.hyperparameters);
            job.progress = 40;
            // Train model
            this.logger.info('Training model', { jobId });
            const trainingMetrics = await this.trainModel(model, trainingSet);
            job.progress = 70;
            // Validate model
     ...
     */
    prepareTrainingData(data, validationSplit) {
        // Shuffle data
     ...
     */
    async initializeModel(domain, hyperparameters) {
...
//# sourceMappingURL=MLModelTrainer.js.map