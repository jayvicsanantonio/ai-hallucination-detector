...
class PerformanceMonitor {
    constructor(cacheManager, connectionPool, thresholds) {
        ...
        this.maxRequestTimesHistory = 10000;
    ...
    recordRequest(responseTime, isError = false) {
        ...
        if (this.requestTimes.length > this.maxRequestTimesHistory) {
            this.requestTimes = this.requestTimes.slice(-this.maxRequestTimesHistory / 2);
        }
    }
    ...
    async collectMetrics() {
        try {
            ...
            const metrics = {
                ...
                databaseStats: {
                    activeConnections: databaseStats.totalConnections -
    ...
    getPerformanceSummary(timeRange) {
        ...
        if (timeRange) {
            relevantMetrics = this.metrics.filter((m) => m.timestamp >= timeRange.start &&
        ...
        const summary = {
            timeRange: {
                ...
                duration: relevantMetrics[relevantMetrics.length - 1].timestamp.getTime() -
                    relevantMetrics[0].timestamp.getTime(),
            },
            requests: {
                total: relevantMetrics[relevantMetrics.length - 1].requestCount,
                averageResponseTime: this.calculateAverage(relevantMetrics.map((m) => m.averageResponseTime)),
                maxResponseTime: Math.max(...relevantMetrics.map((m) => m.averageResponseTime)),
                minResponseTime: Math.min(...relevantMetrics.map((m) => m.averageResponseTime)),
                averageThroughput: this.calculateAverage(relevantMetrics.map((m) => m.throughput)),
            },
            errors: {
                averageErrorRate: this.calculateAverage(relevantMetrics.map((m) => m.errorRate)),
                maxErrorRate: Math.max(...relevantMetrics.map((m) => m.errorRate)),
            },
            resources: {
                averageMemoryUsage: this.calculateAverage(relevantMetrics.map((m) => m.memoryUsage.percentage)),
                maxMemoryUsage: Math.max(...relevantMetrics.map((m) => m.memoryUsage.percentage)),
                averageCpuUsage: this.calculateAverage(relevantMetrics.map((m) => m.cpuUsage)),
                maxCpuUsage: Math.max(...relevantMetrics.map((m) => m.cpuUsage)),
            },
            cache: {
                averageHitRate: this.calculateAverage(relevantMetrics.map((m) => m.cacheStats.hitRate)),
                totalCacheRequests: relevantMetrics[relevantMetrics.length - 1].cacheStats
                    .totalRequests,
            },
            database: {
                averageQueryTime: this.calculateAverage(relevantMetrics.map((m) => m.databaseStats.averageQueryTime)),
                maxQueryTime: Math.max(...relevantMetrics.map((m) => m.databaseStats.averageQueryTime)),
                totalSlowQueries: relevantMetrics[relevantMetrics.length - 1].databaseStats
                    .slowQueries,
            },
        };
        return summary;
    ...
    checkAlerts(metrics) {
        ...
        else if (metrics.averageResponseTime >
        ...
        else if (metrics.cacheStats.hitRate <
        ...
        // Memory usage alerts
        if (metrics.memoryUsage.percentage >
        ...
        else if (metrics.memoryUsage.percentage >
        ...
        if (metrics.databaseStats.averageQueryTime >
        ...
        else if (metrics.databaseStats.averageQueryTime >
    ...
    getOptimizationRecommendations() {
        ...
        if (latestMetrics.cacheStats.hitRate < 80) {
            recommendations.push('Consider increasing cache TTL or warming cache with frequently accessed data');
        }
        if (latestMetrics.databaseStats.averageQueryTime > 100) {
            recommendations.push('Review database queries for optimization opportunities and consider adding indexes');
        }
        if (latestMetrics.memoryUsage.percentage > 80) {
            recommendations.push('Consider increasing memory allocation or implementing memory optimization strategies');
        }
        if (latestMetrics.averageResponseTime > 2000) {
            recommendations.push('Response times are high - consider horizontal scaling or performance optimization');
        }
    ...
    startPeriodicCollection() {
        setInterval(async () => {
            try {
                ...
                this.logger.error('Error in periodic metrics collection:', error);
            }
