...
exports.HIPAAComplianceChecker = void 0;
class HIPAAComplianceChecker {
    ...
    async checkHIPAACompliance(content, medicalData) {
        const issues = [];
        // Check for PII exposure
        issues.push(...this.detectPII(content));
        // Check for patient identifiers
        issues.push(...this.checkPatientIdentifiers(medicalData, content));
        ...
        issues.push(...this.checkMinimumNecessaryStandard(content, medicalData));
        return issues;
    }
    ...
    checkPatientIdentifiers(medicalData, content) {
        const issues = [];
        const patientIds = medicalData.filter((d) => d.type === 'patient_identifier');
        for (const patientId of patientIds) {
            // Check if patient ID is properly protected
            const surroundingText = content.extractedText.substring(Math.max(0, patientId.location.start - 50), Math.min(content.extractedText.length, patientId.location.end + 50));
            const hasProtection = /(?:redacted|anonymized|protected|de-identified)/i.test(surroundingText);
            if (!hasProtection) {
                issues.push({
    ...
    checkRequiredDisclosures(content) {
        const issues = [];
        const text = content.extractedText.toLowerCase();
        // Check for required HIPAA disclosures
    ...
    checkDataSecurityRequirements(content) {
        const issues = [];
        const text = content.extractedText.toLowerCase();
        ...
        const hasSecurityMention = securityTerms.some((term) => text.includes(term));
        if (!hasSecurityMention && this.containsHealthInformation(text)) {
            issues.push({
                ...
                severity: 'medium',
                location: { start: 0, end: 100, line: 1, column: 1 },
                description: 'Document lacks mention of required security safeguards',
    ...
    checkMinimumNecessaryStandard(content, medicalData) {
        const issues = [];
        ...
        if (medicalData.length > 10) {
            issues.push({
                ...
                severity: 'medium',
                location: { start: 0, end: 100, line: 1, column: 1 },
                description: 'Potentially excessive PHI disclosure - review minimum necessary standard',
                evidence: [
    ...
    containsHealthInformation(text) {
        const healthTerms = [
            'patient',
            'medical',
            'health',
        ...
        const hipaaRules = [
            {
                id: 'hipaa-001',
                name: 'pii-protection',
                description: 'Personal identifiers must be protected or anonymized',
                severity: 'critical',
                isActive: true,
                createdAt: new Date(),
                updatedAt: new Date(),
    ...
    initializePIIPatterns() {
        ...
        // Phone numbers
        this.piiPatterns.set('phone', /\b(?:\+1[-.\s]?)?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}\b/g);
        // Email addresses
        this.piiPatterns.set('email', /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g);
        // Credit card numbers (basic pattern)
        this.piiPatterns.set('credit_card', /\b(?:\d{4}[-\s]?){3}\d{4}\b/g);
        // Medical record numbers
        this.piiPatterns.set('mrn', /\b(?:MRN|mrn|medical record|patient id)[\s#:]*(\d{6,})\b/gi);
        // Dates of birth
        this.piiPatterns.set('dob', /\b(?:dob|date of birth|born)[\s:]*(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2})\b/gi);
        // Full names (basic pattern - may have false positives)
        this.piiPatterns.set('full_name', /\b[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\b/g);
        // Addresses (basic pattern)
        this.piiPatterns.set('address', /\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln|Boulevard|Blvd)\b/gi);
    }
}
exports.HIPAAComplianceChecker = HIPAAComplianceChecker;
//# sourceMappingURL=HIPAAComplianceChecker.js.map