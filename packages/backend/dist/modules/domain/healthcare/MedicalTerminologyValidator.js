...
exports.MedicalTerminologyValidator = void 0;
class MedicalTerminologyValidator {
    ...
    async validateTerminology(content, medicalData) {
        const issues = [];
    ...
    validateMedicalTerms(content) {
        const issues = [];
        const text = content.extractedText;
        // Look for potential medical terms and validate them
        const medicalTermPattern = /\b[A-Z][a-z]+(?:itis|osis|emia|uria|pathy|trophy|plasia|scopy|tomy|ectomy)\b/g;
        let match;
    ...
    validateDosageFormats(medicalData) {
        const issues = [];
    ...
    checkTerminologyConsistency(content) {
        ...
        for (const variant of terminologyVariants) {
            const foundTerms = variant.terms.filter((term) => text.includes(term));
    ...
    validateMedicalAbbreviations(content) {
        const issues = [];
        const text = content.extractedText;
    ...
    validateDosageFormat(medication) {
        if (!medication.dosage)
            return null;
        // Check for proper dosage format
        const validDosagePattern = /^\d+(?:\.\d+)?\s*(?:mg|g|ml|mcg|units?|tablets?|capsules?)(?:\s+(?:once|twice|three times|four times)\s+(?:daily|per day|a day))?$/i;
        ...
        const terminologyRules = [
            {
                ...
                description: 'Medical terms must be correctly spelled and recognized',
                severity: 'medium',
                isActive: true,
                createdAt: new Date(),
                updatedAt: new Date(),
            },
            {
                ...
                description: 'Dosage formats must follow standard medical conventions',
                severity: 'high',
                isActive: true,
                createdAt: new Date(),
                updatedAt: new Date(),
            },
            {
                ...
                description: 'Dangerous medical abbreviations should be avoided',
                severity: 'medium',
                isActive: true,
                createdAt: new Date(),
                updatedAt: new Date(),
...
exports.MedicalTerminologyValidator = MedicalTerminologyValidator;
//# sourceMappingURL=MedicalTerminologyValidator.js.map