...
exports.ContractTermValidator = void 0;
class ContractTermValidator {
    constructor() {
        this.rules = new Map();
        this.initializeContractRules();
    }
    async validateContractTerms(content, legalEntities) {
        const issues = [];
        ...
        issues.push(...this.validateEssentialElements(contractTerms, content));
        // Validate party identification
        issues.push(...this.validateParties(contractTerms, legalEntities, content));
        // Validate consideration clauses
        issues.push(...this.validateConsideration(contractTerms, content));
        // Validate termination clauses
        issues.push(...this.validateTerminationClauses(contractTerms, content));
        // Validate legal capacity
        issues.push(...this.validateLegalCapacity(contractTerms, legalEntities, content));
        return issues;
    }
    async validateTerms(terms) {
    ...
    extractContractTerms(content) {
        const terms = [];
        const text = content.extractedText;
        // Extract parties
        const partyMatches = text.matchAll(/(?:party|parties|between|among)\s+([^.]+)/gi);
        ...
        const considerationMatches = text.matchAll(/(?:consideration|payment|compensation|fee)\s+([^.]+)/gi);
        ...
        // Extract obligations
        const obligationMatches = text.matchAll(/(?:shall|must|will|agrees?\s+to)\s+([^.]+)/gi);
    ...
    validateEssentialElements(terms, content) {
        const issues = [];
    ...
    validateParties(terms, entities, content) {
        const issues = [];
        ...
        for (const partyTerm of partyTerms) {
            if (partyTerm.content.length < 10) {
                issues.push({
                    ...
                    description: 'Party identification is too vague or incomplete',
    ...
    validateConsideration(terms, content) {
        const issues = [];
        const considerationTerms = terms.filter((term) => term.type === 'consideration');
        ...
        for (const consideration of considerationTerms) {
            if (!this.hasSpecificAmount(consideration.content)) {
                issues.push({
                    ...
                    description: 'Consideration amount or terms are not specific enough',
    ...
    validateTerminationClauses(terms, content) {
        const issues = [];
        const text = content.extractedText.toLowerCase();
        const hasTerminationClause = /(?:terminat|end|expir|cancel)/i.test(text);
    ...
    validateLegalCapacity(terms, entities, content) {
        const issues = [];
        ...
        for (const entity of entities) {
            if (entity.type === 'individual' &&
                ...
                issues.push({
                    ...
                    description: 'Individual party may lack clear legal capacity indicators',
    ...
    validateTermSpecificRequirements(term) {
        const issues = [];
        switch (term.type) {
            case 'party':
                if (!this.isValidPartyIdentification(term.content)) {
                    issues.push({
                        ...
                        description: 'Party identification does not meet standard requirements',
                        evidence: [term.content],
                        confidence: 75,
                        moduleSource: 'contract-term-validator',
                    });
            ...
            case 'consideration':
                if (!this.isValidConsideration(term.content)) {
                    issues.push({
                        ...
                        severity: 'high',
                        location: term.location,
                        description: 'Consideration clause is invalid or insufficient',
                        evidence: [term.content],
                        confidence: 80,
                        moduleSource: 'contract-term-validator',
                    });
...
exports.ContractTermValidator = ContractTermValidator;
//# sourceMappingURL=ContractTermValidator.js.map