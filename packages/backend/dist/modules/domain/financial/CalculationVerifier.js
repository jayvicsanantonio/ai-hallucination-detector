...
exports.CalculationVerifier = void 0;
class CalculationVerifier {
    constructor() {
        this.PRECISION_TOLERANCE = 0.01;
    }
    async verifyCalculations(content, financialData) {
        const issues = [];
    ...
    extractCalculations(text) {
        ...
        const simpleArithmeticPattern = /(\$?[\d,]+(?:\.\d{2})?)\s*([+\-*/])\s*(\$?[\d,]+(?:\.\d{2})?)\s*=\s*(\$?[\d,]+(?:\.\d{2})?)/g;
        let match;
        while ((match = simpleArithmeticPattern.exec(text)) !== null) {
            ...
            calculations.push({
                ...
                expectedResult: this.calculateExpectedResult(this.parseNumber(operand1), operator, this.parseNumber(operand2)),
                location: {
                    start: match.index,
        ...
        const percentagePattern = /(\$?[\d,]+(?:\.\d{2})?)\s*\*\s*(\d+(?:\.\d+)?%)\s*=\s*(\$?[\d,]+(?:\.\d{2})?)/g;
        while ((match = percentagePattern.exec(text)) !== null) {
            ...
            calculations.push({
                ...
                result: this.parseNumber(result),
                expectedResult: this.parseNumber(base) *
        ...
        const compoundInterestPattern = /(\$?[\d,]+(?:\.\d{2})?)\s*\*?\s*\(1\s*\+\s*(\d+(?:\.\d+)?%?)\)\s*\^\s*(\d+)\s*=\s*(\$?[\d,]+(?:\.\d{2})?)/g;
        while ((match = compoundInterestPattern.exec(text)) !== null) {
            ...
            const expectedResult = this.parseNumber(principal) *
    ...
    verifyCalculation(calculation) {
        ...
        if (difference > this.PRECISION_TOLERANCE) {
            return {
                isValid: false,
                error: `Calculation result differs by ${difference.toFixed(4)} from expected value`,
            };
        }
        return { isValid: true };
    }
    verifyFinancialRatios(financialData) {
        const issues = [];
        ...
        for (const ratio of ratios) {
            ...
            if (ratio.numericValue > 1000) {
                issues.push({
                    ...
                    severity: 'low',
                    location: ratio.location,
                    description: 'Extremely high ratio value - please verify calculation',
                    evidence: [ratio.value],
                    confidence: 70,
                    moduleSource: 'calculation-verifier',
                });
    ...
    verifyPercentageCalculations(text) {
        const issues = [];
        // Look for percentage change calculations
        const percentageChangePattern = /\((\$?[\d,]+(?:\.\d{2})?)\s*-\s*(\$?[\d,]+(?:\.\d{2})?)\)\s*\/\s*(\$?[\d,]+(?:\.\d{2})?)\s*\*\s*100\s*=\s*(\d+(?:\.\d+)?%?)/g;
        let match;
        while ((match = percentageChangePattern.exec(text)) !== null) {
            const [fullMatch, newValue, oldValue, baseValue, result] = match;
    ...
    verifyCompoundCalculations(text) {
        const issues = [];
        // Look for present value calculations: FV / (1+r)^t = PV
        const presentValuePattern = /(\$?[\d,]+(?:\.\d{2})?)\s*\/\s*\(1\s*\+\s*(\d+(?:\.\d+)?%?)\)\s*\^\s*(\d+)\s*=\s*(\$?[\d,]+(?:\.\d{2})?)/g;
        let match;
        while ((match = presentValuePattern.exec(text)) !== null) {
            const [fullMatch, futureValue, rate, time, presentValue] = match;
            ...
            if (Math.abs(actualPV - expectedPV) > this.PRECISION_TOLERANCE) {
                issues.push({
    ...
    calculateExpectedResult(operand1, operator, operand2) {
        switch (operator) {
            case '+':
...
exports.CalculationVerifier = CalculationVerifier;
//# sourceMappingURL=CalculationVerifier.js.map