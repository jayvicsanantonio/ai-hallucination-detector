...
exports.CoherenceValidator = void 0;
class CoherenceValidator {
    ...
    async validateCoherence(content) {
        const issues = [];
        try {
            ...
            const semanticIssues = await this.validateSemanticCoherence(content);
            issues.push(...semanticIssues);
            // Validate temporal consistency
            const temporalIssues = await this.validateTemporalConsistency(content);
            issues.push(...temporalIssues);
            // Validate causal consistency
            const causalIssues = await this.validateCausalConsistency(content);
            issues.push(...causalIssues);
            // Validate cross-references
            const referenceIssues = await this.validateCrossReferences(content);
    ...
    async validateSemanticCoherence(content) {
        const issues = [];
        const text = content.extractedText;
        ...
        for (let i = 0; i < sentences.length - 1; i++) {
            ...
            const semanticSimilarity = this.calculateSemanticSimilarity(currentSentence.text, nextSentence.text);
            // If semantic similarity is very low, it might indicate incoherence
            if (semanticSimilarity < 0.05 &&
                ...
                issues.push({
                    ...
                    context: this.getContext(text, nextSentence.location.start, 100),
                });
            }
        }
        // Check for contradictory semantic content
        const contradictoryPairs = this.findSemanticContradictions(sentences);
        contradictoryPairs.forEach((pair) => {
            issues.push({
                ...
                context: this.getContext(text, pair.sentence2.location.start, 100),
            });
        });
        return issues;
    }
    async validateTemporalConsistency(content) {
        const issues = [];
        const text = content.extractedText;
        ...
        for (let i = 0; i < temporalEvents.length; i++) {
            for (let j = i + 1; j < temporalEvents.length; j++) {
                ...
                const inconsistency = this.checkTemporalInconsistency(event1, event2);
                if (inconsistency) {
                    issues.push({
                        ...
                        context: this.getContext(text, event2.location.start, 100),
                    });
                }
            }
        }
    ...
    async validateCausalConsistency(content) {
        const issues = [];
        const text = content.extractedText;
        ...
        for (let i = 0; i < causalRelations.length; i++) {
            for (let j = i + 1; j < causalRelations.length; j++) {
                ...
                const inconsistency = this.checkCausalInconsistency(relation1, relation2);
                if (inconsistency) {
                    issues.push({
                        ...
                        context: this.getContext(text, relation2.effectLocation.start, 100),
                    });
                }
            }
        }
    ...
    async validateCrossReferences(content) {
        const issues = [];
        const text = content.extractedText;
        // Extract cross-references
        const crossReferences = this.extractCrossReferences(text, content.entities);
        ...
        crossReferences.forEach((reference) => {
            if (!reference.isResolved) {
                issues.push({
                    ...
                    context: this.getContext(text, reference.location.start, 100),
                });
            }
        });
        return issues;
    ...
    isSignificantTopicShift(sentence1, sentence2) {
    ...
    findSemanticContradictions(sentences) {
        ...
        for (let i = 0; i < sentences.length; i++) {
            for (let j = i + 1; j < sentences.length; j++) {
                ...
                const topicSimilarity = this.calculateSemanticSimilarity(sent1.text, sent2.text);
    ...
    extractCausalRelations(text) {
        ...
        sentences.forEach((sentence) => {
            this.causalIndicators.forEach((indicator) => {
                if (sentence.text.toLowerCase().includes(indicator)) {
                    const parts = sentence.text.split(new RegExp(indicator, 'i'));
                    if (parts.length === 2) {
                        relations.push({
                            id: `causal_${Date.now()}_${Math.random()}`,
                            cause: parts[0].trim(),
    ...
    extractCrossReferences(text, entities) {
        const references = [];
        const sentences = this.extractSentences(text);
        sentences.forEach((sentence) => {
            this.referencePronouns.forEach((pronoun) => {
                ...
                while ((match = regex.exec(sentence.text)) !== null) {
                    ...
                    reference.isResolved = this.tryResolveReference(reference, entities, sentences);
    ...
    checkTemporalInconsistency(event1, event2) {
    ...
    checkCausalInconsistency(relation1, relation2) {
    ...
    classifyReferenceType(pronoun) {
        const pronounLower = pronoun.toLowerCase();
        if (['he', 'she', 'it', 'they', 'him', 'her', 'them'].includes(pronounLower)) {
            return 'pronoun';
        }
    ...
    tryResolveReference(reference, entities, sentences) {
        // Simple resolution: look for entities in previous sentences
        const relevantEntities = entities.filter((entity) => entity.location.start < reference.location.start &&
...
exports.CoherenceValidator = CoherenceValidator;
//# sourceMappingURL=CoherenceValidator.js.map