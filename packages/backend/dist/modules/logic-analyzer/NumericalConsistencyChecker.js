...
exports.NumericalConsistencyChecker = void 0;
class NumericalConsistencyChecker {
    constructor() {
        this.numberPatterns = {
            ...
            measurement: /\b\d+(?:\.\d+)?\s*(?:kg|lb|m|ft|cm|in|km|mi|l|gal|oz|g)\b/gi,
        };
    ...
    async checkNumericalConsistency(content) {
        ...
        try {
            ...
            const calculationErrors = await this.checkCalculationErrors(text);
            inconsistencies.push(...calculationErrors);
            // Check unit mismatches
            const unitMismatches = this.checkUnitMismatches(numericalValues);
            inconsistencies.push(...unitMismatches);
            // Check range violations
            const rangeViolations = this.checkRangeViolations(numericalValues, text);
            inconsistencies.push(...rangeViolations);
            // Check sum mismatches (for financial content)
            const sumMismatches = this.checkSumMismatches(numericalValues, text);
            inconsistencies.push(...sumMismatches);
            // Check percentage consistency
            const percentageErrors = this.checkPercentageConsistency(numericalValues, text);
    ...
    extractNumericalValues(text) {
        ...
        Object.entries(this.numberPatterns).forEach(([type, pattern]) => {
            ...
            while ((match = regex.exec(text)) !== null) {
                ...
                const value = this.parseNumericalValue(originalText, type);
                if (value !== null) {
                    values.push({
                        value,
    ...
    async checkCalculationErrors(text) {
        const errors = [];
        for (const pattern of this.calculationPatterns) {
            ...
            while ((match = regex.exec(text)) !== null) {
                ...
                const tolerance = Math.max(0.01, Math.abs(expectedResult) * 0.001);
                if (Math.abs(expectedResult - statedResult) > tolerance) {
                    errors.push({
                        id: `calc_error_${Date.now()}_${Math.random()}`,
                        type: 'calculation_error',
                        description: `Calculation error: ${operand1} ${operator} ${operand2} should equal ${expectedResult.toFixed(2)}, not ${statedResult}`,
                        location: {
                            start: match.index,
                        ...
                        severity: this.calculateSeverityForCalculationError(expectedResult, statedResult),
                        confidence: 95,
                    });
                }
            }
    ...
    checkRangeViolations(values, text) {
        const errors = [];
        ...
        ranges.forEach((range) => {
            ...
            const relevantValues = values.filter((value) => this.isValueRelevantToRange(value, range));
            relevantValues.forEach((value) => {
                if (value.value < range.min || value.value > range.max) {
                    errors.push({
                        ...
                        severity: this.calculateSeverityForRangeViolation(value.value, range),
                        confidence: 70,
                    });
                }
            });
    ...
    checkSumMismatches(values, text) {
        const errors = [];
        ...
        sumPatterns.forEach((pattern) => {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                const contributingValues = values.filter((value) => value.location.start < match.index &&
                ...
                if (contributingValues.length > 1) {
                    const calculatedSum = contributingValues.reduce((sum, val) => sum + val.value, 0);
                    ...
                    if (Math.abs(calculatedSum - statedTotal) > tolerance) {
                        errors.push({
                            ...
                            type: 'sum_mismatch',
                            description: `Sum mismatch: stated total ${statedTotal} doesn't match calculated sum ${calculatedSum.toFixed(2)}`,
                            location: {
                                start: match.index,
                                end: match.index + match[0].length,
                                line: this.getLineNumber(text, match.index),
                            ...
                            severity: this.calculateSeverityForSumMismatch(calculatedSum, statedTotal),
                            confidence: 75,
                        });
                    }
                }
    ...
    checkPercentageConsistency(values, text) {
        const errors = [];
                ...
                if (this.shouldSumToHundred(group, text)) {
                    ...
                    if (Math.abs(sum - 100) > tolerance) {
                        errors.push({
                            ...
                            type: 'calculation_error',
                            description: `Percentages should sum to 100% but sum to ${sum.toFixed(1)}%`,
    ...
    haveSimilarContext(value1, value2) {
        // Check if values are close in position
        const distance = Math.abs(value1.location.start - value2.location.start);
        if (distance > 200)
            return false;
        // Check for common keywords in context
        const context1Words = new Set(value1.context.toLowerCase().split(/\s+/));
        const context2Words = new Set(value2.context.toLowerCase().split(/\s+/));
        const intersection = new Set([...context1Words].filter((x) => context2Words.has(x)));
        const union = new Set([...context1Words, ...context2Words]);
        return intersection.size / union.size > 0.3;
    }
    getUnitType(unit) {
        const lowerUnit = unit.toLowerCase();
        for (const [type, conversions] of Object.entries(this.unitConversions)) {
    ...
    isValueRelevantToRange(value, range) {
        // Check if value is mentioned in context of the range
        const distance = Math.abs(value.location.start - range.location.start);
        return distance < 100; // Within 100 characters
    }
    shouldSumToHundred(percentages, text) {
    ...
    calculateSeverityForCalculationError(expected, actual) {
        const percentError = Math.abs((expected - actual) / expected) * 100;
        if (percentError > 50)
            return 'critical';
        if (percentError > 20)
            return 'high';
    ...
    calculateSeverityForRangeViolation(value, range) {
        const rangeSize = range.max - range.min;
        const violation = Math.min(Math.abs(value - range.min), Math.abs(value - range.max));
    ...
    calculateSeverityForSumMismatch(expected, actual) {
        const percentError = Math.abs((expected - actual) / expected) * 100;
        if (percentError > 25)
            return 'high';
        if (percentError > 10)
...
exports.NumericalConsistencyChecker = NumericalConsistencyChecker;
//# sourceMappingURL=NumericalConsistencyChecker.js.map