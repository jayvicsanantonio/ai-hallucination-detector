...
exports.SourceCredibilityScorer = void 0;
class SourceCredibilityScorer {
    ...
    assessCredibility(source, domain) {
        const factors = {
            sourceType: this.scoreSourceType(source.sourceType || 'other'),
            recency: this.scoreRecency(source.publishDate, source.lastVerified),
    ...
    updateCredibilityFromFeedback(currentScore, feedback, feedbackWeight = 1.0) {
        const adjustment = feedback === 'positive'
    ...
    compareSourceCredibility(sources, domain) {
        const assessments = sources.map((source) => ({
            source,
            assessment: this.assessCredibility(source, domain),
        }));
        return assessments
            .sort((a, b) => b.assessment.overallScore - a.assessment.overallScore)
            .map((item) => item.source);
    }
    scoreSourceType(sourceType) {
        return this.sourceTypeWeights[sourceType] || 50;
    }
    scoreRecency(publishDate, lastVerified) {
        const now = new Date();
        const relevantDate = lastVerified || publishDate;
        if (!relevantDate)
            return 30; // Low score for unknown dates
        const ageInDays = (now.getTime() - relevantDate.getTime()) /
            (1000 * 60 * 60 * 24);
        if (ageInDays <= 30)
        ...
        if (ageInDays <= 90)
            return 90; // Recent
        if (ageInDays <= 365)
            return 75; // Within a year
        if (ageInDays <= 1095)
            return 60; // Within 3 years
        if (ageInDays <= 1825)
            return 40; // Within 5 years
    ...
    scoreAuthor(author) {
        ...
        if (institutionalKeywords.some((keyword) => author.includes(keyword)))
            return 80;
        return 60; // Default for named authors
    }
    scoreDomainRelevance(source, domain) {
        if (!domain || !source.url)
            return 70; // Neutral score
        const domainPatterns = this.domainAuthorityPatterns[domain];
        if (!domainPatterns)
            return 70;
        const isAuthoritative = domainPatterns.some((pattern) => source.url?.toLowerCase().includes(pattern.toLowerCase()));
        return isAuthoritative ? 95 : 60;
    }
    scoreVerificationHistory(source) {
    ...
    calculateOverallScore(factors) {
        ...
        return Math.round(factors.sourceType * weights.sourceType +
    ...
    generateReasoning(factors, source) {
        const reasoning = [];
        if (factors.sourceType >= 90) {
            reasoning.push(`High credibility source type: ${source.sourceType}`);
        }
        else if (factors.sourceType <= 60) {
            reasoning.push(`Lower credibility source type: ${source.sourceType}`);
        }
    ...
    calculateConfidence(factors) {
        ...
        let confidence = 50;
        if (factors.authorCredibility > 50)
            confidence += 15;
        if (factors.recency > 70)
            confidence += 15;
        if (factors.domainRelevance > 80)
            confidence += 10;
        if (factors.verificationHistory > 70)
            confidence += 10;
        return Math.min(95, confidence);
    }
}
exports.SourceCredibilityScorer = SourceCredibilityScorer;
//# sourceMappingURL=SourceCredibilityScorer.js.map