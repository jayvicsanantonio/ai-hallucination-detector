...
class ClaimExtractor {
        ...
        for (const pattern of this.factualPatterns) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (this.isValidClaim(statement)) {
                    const location = this.getLocation(text, match.index, match[0].length);
                    const context = this.getContext(text, match.index, 100);
                    const claimType = this.classifyClaimType(statement, domain);
                    const confidence = this.calculateExtractionConfidence(statement, domain);
                    claims.push({
                        statement,
                        confidence,
                        location,
    ...
    isValidClaim(statement) {
        ...
        if (!verbs.some((verb) => statement.toLowerCase().includes(verb))) {
            return false;
        }
        return true;
    }
    getLocation(text, start, length) {
    ...
    classifyClaimType(statement, domain) {
        const lowerStatement = statement.toLowerCase();
        // Statistical patterns
        if (lowerStatement.match(/\d+(?:\.\d+)?%/) ||
            lowerStatement.includes('percent')) {
            return 'statistical';
        }
        // Regulatory patterns
        if (lowerStatement.match(/(?:requires?|prohibits?|mandates?|complies?|violates?)/)) {
            return 'regulatory';
        }
    ...
    calculateExtractionConfidence(statement, domain) {
        ...
        if (domain) {
            const keywords = this.domainKeywords[domain];
            const keywordCount = keywords.filter((keyword) => statement.toLowerCase().includes(keyword)).length;
    ...
    deduplicateClaims(claims) {
        ...
        for (const claim of claims) {
            const isDuplicate = deduplicated.some((existing) => {
                ...
                const overlap = Math.max(0, Math.min(existing.location.end, claim.location.end) -
                    Math.max(existing.location.start, claim.location.start));
                const overlapRatio = overlap /
                    Math.min(existing.location.end - existing.location.start, claim.location.end - claim.location.start);
