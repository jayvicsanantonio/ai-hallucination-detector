...
exports.RegulatoryReferenceManager = void 0;
/**
...
class RegulatoryReferenceManager {
     ...
     */
    async getRegulatoryReferences(rule) {
        const references = [];
        ...
        for (const [docId, document] of this.documents) {
            if (this.isDocumentApplicable(document, rule)) {
                const relevantSections = this.findRelevantSections(document, rule);
                const applicabilityScore = this.calculateApplicabilityScore(document, rule);
                if (applicabilityScore > 30) {
                    ...
                    references.push({
                        ...
                        document,
                        relevantSections,
                        citationText: this.generateCitation(document, relevantSections),
        ...
        return references.sort((a, b) => b.applicabilityScore - a.applicabilityScore);
    }
    /**
     * Get compliance guidance for a rule
     */
    async getComplianceGuidance(ruleId) {
        return this.guidance.get(ruleId) || null;
    }
    /**
     * Generate a comprehensive regulatory context for a violation
     */
    async generateRegulatoryContext(rule, violationContext) {
        const primaryReferences = await this.getRegulatoryReferences(rule);
        const guidance = await this.getComplianceGuidance(rule.id);
        const relatedUpdates = this.getRelatedUpdates(rule.regulation);
        const enforcementHistory = this.getEnforcementHistory(rule.regulation);
     ...
     */
    async addRegulatoryDocument(document) {
     ...
     */
    async updateComplianceGuidance(ruleId, guidance) {
        const fullGuidance = {
            ...guidance,
            ruleId,
     ...
     */
    async addRegulatoryUpdate(update) {
     ...
     */
    getRelatedUpdates(regulation, limit = 5) {
        return this.updates
            .filter((update) => update.regulation === regulation ||
            update.title
                .toLowerCase()
     ...
     */
    searchDocuments(keywords, domain, jurisdiction) {
        const results = [];
        for (const [, document] of this.documents) {
            ...
            for (const keyword of keywords) {
                ...
                if (document.description.toLowerCase().includes(lowerKeyword)) {
                    score += 5;
                }
                if (document.keywords.some((k) => k.toLowerCase().includes(lowerKeyword))) {
                    score += 8;
                }
            }
            if (score > 0) {
     ...
     */
    generateCitation(document, sections) {
    ...
    initializeRegulatoryDocuments() {
        ...
        const documents = [
            ...
            },
            {
                ...
                description: 'Federal law that establishes auditing and financial regulations for public companies',
    ...
    initializeComplianceGuidance() {
        // Initialize with sample compliance guidance
        const guidanceItems = [
            {
                ruleId: 'hipaa-phi-disclosure',
                regulation: 'HIPAA',
                guidance: 'Protected Health Information (PHI) must not be disclosed without proper authorization or legal basis.',
                examples: [
    ...
    isDocumentApplicable(document, rule) {
    ...
    findRelevantSections(document, rule) {
        const sections = [];
        ...
        const matchingKeywords = ruleKeywords.filter((rk) => docKeywords.some((dk) => dk.includes(rk) || rk.includes(dk)));
    ...
    calculateApplicabilityScore(document, rule) {
        let score = 0;
        ...
        const ruleKeywords = rule.keywords.map((k) => k.toLowerCase());
        const docKeywords = document.keywords.map((k) => k.toLowerCase());
        const keywordMatches = ruleKeywords.filter((rk) => docKeywords.some((dk) => dk.includes(rk) || rk.includes(dk)));
        score += keywordMatches.length * 5;
        // Bonus for recent documents
        const daysSinceUpdate = (Date.now() - document.lastUpdated.getTime()) /
            (1000 * 60 * 60 * 24);
    ...
    getEnforcementHistory(regulation) {
...
exports.RegulatoryReferenceManager = RegulatoryReferenceManager;
//# sourceMappingURL=RegulatoryReferenceManager.js.map